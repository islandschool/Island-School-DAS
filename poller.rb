...
require 'eeml'
require 'c:\\das\\config\\environment'
require 'c:\\das\\sda_utility'
require 'c:\\das\\modbus_device'
...
class Poller
  ...
  def poll
    ...
    pconf = YAML::load(File.open('c:/das/config/pachube.yml'))
    ...
    loop do
      ...
      begin
        ...
        # TIP: add other transducers here
       ...
       is_con = EnergyReading.new(:frequency => pt1[0],
        ...
        # send zero to pachube if no data
        if is_bec.nil?
      ...
      # TODO: more cases of exception handling
      rescue Exception => e  
        unless e.nil? 
          puts e.message
          puts e.backtrace
